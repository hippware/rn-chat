...
- (void)loadView {
    ...
    _containerView = [[ADTransitionView alloc] initWithFrame:CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y, self.view.frame.size.width, self.view.frame.size.height)];
...
- (UIViewController *)popViewControllerWithTransition:(ADTransition *)transition {
    if (self.viewControllers.count < 2 || _isContainerViewTransitioning) {
        return nil;
    }
    BOOL animated = transition ? YES : NO;
...
- (NSArray *)popToViewController:(UIViewController *)viewController withTransition:(ADTransition *)transition {
    ...
    if (indexInViewController == NSNotFound || _isContainerViewTransitioning) {
        return nil;
    }
