...
NSString *DoTest(NSString *publicKeyPath, NSString *textPath, NSString *resultsPath)
{
    NSError  *error;
    NSString *signatureStr = [NSString stringWithContentsOfFile:resultsPath encoding:NSUTF8StringEncoding error:&error];
    NSData *signature = [[NSData alloc] initWithBase64EncodedString:signatureStr options:0];
    NSData *hash = GetSHA256Hash([NSData dataWithContentsOfFile:textPath]);
    Verifier *verifier = [[Verifier alloc] initWithContentsOfFile:publicKeyPath tag:@"com.iccir.SignAndVerify.public-key"];

    if (![verifier verifySHA256Hash:hash withSignature:signature]) {
        NSLog(@"OS X Verifier failed to verify line");
    }
    return @"";
}
