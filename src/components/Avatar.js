...
export default class Avatar extends Component {
    ...
    render() {
        ...
        return <Clazz style={{justifyContent: 'flex-end'}}
                      onPress={profile && !profile.isOwn ? () => statem.logged.profileDetailsContainer({
                          parent: '_home',
                          item: profile.user
                      }) : null}>
            <View ref={component => this._root = component} style={[style, {height: size * k, width: size * k}]}>
                ...
                {showFrame && <View style={{
                    ...
                    style={{width: size * k, height: size * k}}/></View>}
                {profile && !profile.isOwn && !disableStatus && <View style={{
