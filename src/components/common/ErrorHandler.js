...
class ErrorHandler extends React.Component<{}> {
  ...
  render() {
    return this.error ? <ErrorView error={this.error} errorInfo={this.errorInfo} reload={this.reload} /> : this.props.children;
  }
}
  ...
  if (settings.isStaging) {
    ...
    return (
      <View style={{flex: 1, alignItems: 'center'}}>
        <Image source={require('../../../images/surpriseBot.png')} style={{height: 68, marginTop: 160, marginBottom: 40}} resizeMode='contain' />
        <RText size={30} weight='Light' color={colors.PINK} style={{textAlign: 'center', marginBottom: 30}}>
          {'Oops! something\r\nwent wrong.'}
        </RText>
        <RText size={18} color={colors.DARK_GREY} style={{marginBottom: 50}}>
          Please tap reload.
        </RText>
        <Button style={{position: 'relative', left: 0, right: 0, marginHorizontal: 30, marginTop: 50}} onPress={reload}>
          <View style={{alignItems: 'center', flexDirection: 'row'}}>
            <Image source={require('../../../images/reload.png')} style={{height: 19, marginRight: 5}} resizeMode='contain' />
