...
export default class BotOptions extends React.Component {
    ...
    render() {
        ...
        return <Screen>
            <ShowNotification/>
            <Card>
                {isOwn && <View><Cell onPress={() => statem.logged.botEdit({item: this.bot.id})}
                                      image={require('../../images/edit.png')}>Edit Bot</Cell><Separator
                    width={1}/></View>}
                {((isOwn && this.bot.visibility !== VISIBILITY_OWNER) || this.bot.visibility === VISIBILITY_PUBLIC) &&
                <Cell onPress={() => statem.logged.botShare({item: this.bot.id})}
                      image={require('../../images/iconShare.png')}>Share</Cell>}
                {isOwn && <View><Separator width={1}/><Cell textStyle={{color: 'rgb(254,92,108)'}}
                                                            onPress={() => Alert.alert(null, 'Are you sure you want to delete this bot?', [
                ...
                {!isOwn && this.bot.isSubscribed &&
                <View><Separator width={1}/><Cell textStyle={{color: 'rgb(254,92,108)'}}
