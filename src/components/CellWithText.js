    ...
    render() {
        const cell = <View style={[{
            ...
            <Text
                numberOfLines={1}
                style={[{
                    flex: 1, fontFamily: 'Roboto-Regular', fontSize: 15,
                    color: location.isDay ? navBarTextColorDay : navBarTextColorNight
                },
                    this.props.textStyle]}>{this.props.children}
            </Text>
            {this.props.onRemove && <TouchableOpacity style={{justifyContent: 'center', alignItems: 'center'}}
                                                      onPress={this.props.onRemove}><Image
                source={require('../../images/iconClose.png')}/></TouchableOpacity>}
