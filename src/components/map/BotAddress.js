...
class BotAddress extends React.Component {
  ...
  redirectToLocation = (coords) => {
    ...
    botStore.bot.addressData = new Address();
    botStore.bot.addressData.address = undefined;
    botStore.bot.location = coords;
  ...
  reverseGeoCode = (coords) => {
    geocoding.reverse(coords).then((d) => {
      if (d && d.length) {
        ...
        botStore.bot.addressData.address = d[0].place_name;
      }
    });
  };
