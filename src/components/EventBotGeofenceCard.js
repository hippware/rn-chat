  ...
  render(){
    ...
    return <View style={{paddingTop:15, paddingBottom:10}}>
      <View style={{paddingLeft:19*k, paddingRight:23*k, paddingBottom:12, flexDirection:'row' }}>
        <CardText isDay={isDay}>{`@${eventBot.target.handle}`} {eventBot.isEnter ? 'entered' : 'exited' } {(!eventBot.bot.owner || eventBot.bot.owner.isOwn) ? 'your' : `@${eventBot.bot.owner.handle}'s`} bot</CardText>
      </View>
