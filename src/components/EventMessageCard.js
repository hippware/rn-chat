...
export default class EventMessageCard extends React.Component {
  ...
  render(){
    ...
    return <View style={{paddingTop:15}}>
      {!!msg.from && <View style={{paddingLeft:19*k, paddingRight:23*k }}><CardText isDay={isDay}>{msg.from.isOwn ? 'you' : `@${msg.from.handle}`} sent you a message.
      </CardText></View>}
      {!!msg.body && <View style={{paddingLeft:19*k, paddingRight:23*k, paddingBottom:13.3*k}}><Text style={{fontFamily:'Roboto-Light',color:isDay ? 'rgb(81,67,96)' : 'white',fontSize:15}}>"{msg.body}"</Text></View>}
      {!!msg.media && msg.media.source && <View style={{paddingTop:13.3*k}}><ResizedImage image={msg.media}/></View>}
