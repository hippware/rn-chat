    ...
    render() {
        ...
        return (
            <Card style={[{marginTop: 10}, this.props.style]}
                      ...
                      <View style={{position: 'absolute', top: -5, left: 30 * k, right: 0, height: 40 * k}}>
                          <View style={{flex: 1, flexDirection: 'row'}}>
                              {chat.participants.map(profile =>
                                  <Avatar key={profile.user + 'avatar_event'} size={40 * k}
                                          source={profile.avatar && profile.avatar.source} profile={profile}
                                          title={profile.displayName} isDay={isDay}/>)}
                          </View>
                ...
                {eventChat.isFollowed && (!!msg.body || (msg.media && msg.media.source)) &&
                <View style={{paddingTop: 15, paddingBottom: 15}}>
                    {!!msg.from && <View style={{paddingLeft: 15, paddingRight: 15}}><CardText
                        isDay={isDay}>{msg.from.isOwn ? 'you' : `@${msg.from.handle}`} sent you a message.
                    </CardText></View>}
                    {!!msg.body && <View style={{paddingLeft: 15, paddingRight: 15}}><Text style={{
                        fontFamily: 'Roboto-Light',
                ...
                { !msg.from && profile.isMutual && (
                    <View>
                        ...
                        <TouchableOpacity onPress={() => statem.home.openPrivateChat({item: chat.id})}
                                          style={{justifyContent: 'center', height: 40, flex: 1, alignItems: 'center'}}>
                            <Text style={{
                                fontFamily: 'Roboto-Regular',
                                fontSize: 15,
                                color: 'rgb(254,92,108)',
                                letterSpacing: 0.7
                            }}>Message {profile.displayName}</Text>
                        </TouchableOpacity>
                    </View>
                )}
                ...
                { !eventChat.isFollowed && <View>
                    ...
                    <TouchableOpacity onPress={() => statem.home.follow(profile)} style={{
                        flex: 1,
                        flexDirection: 'row',
