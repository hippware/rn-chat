...
import type {Scene, SceneRendererProps, NavigationState, Layout, Route, SubscriptionName, PagerProps, Style} from './TabViewTypeDefinitions';
...
switch (Platform.OS) {
  case 'android':
    ...
    break;
  case 'ios':
    TabViewPager = require('./TabViewPagerCube').default;
    break;
  default:
...
export default class TabViewAnimated<T: Route<*>> extends PureComponent<DefaultProps<T>, Props<T>, State> {
  static propTypes = {
  ...
  render() {
    ...
    return (
      <View onLayout={this._handleLayout} loaded={this.state.loaded} style={[styles.container, this.props.style]}>
        {renderPager({
          ...props,
          ...rest,
