...
function onPhotoAdd({uploadAvatar}){
    ...
    UIImagePickerManager.showImagePicker(options, (response) => {
        ...
        if (response.didCancel) {
            ...
            uploadAvatar({file: source, width: response.width, height: response.height, size: response.fileSize});
        }
    });
}

const SignUpAvatar = props => {
    const borderWidth = props.avatar ? 2*k : 0;
    const avatarSource = props.avatar || require("../../images/addPhotoLight.png");
    return <TouchableOpacity style={{alignItems:'center'}} onPress={onPhotoAdd.bind(null, props)}>
            <Image style={[{top:70*k,width:82*k,height:80*k, borderRadius:40*k, borderWidth, borderColor:'white'}, props.style]}
                   source={avatarSource}/>
        </TouchableOpacity>
