...
export default class LocationBotAddress extends React.Component {
    ...
    render () {
        ...
        return <View style={{flex: 1}}>
            <Map ref='map' showOnlyBot bot={bot.bot} fullMap={true} followUser={false} showUser
                 location={bot.address.location} isDay={location.isDay} onBoundsDidChange={this.onBoundsDidChange}
            ...
            <View pointerEvents='box-none' style={[{
                ...
                {this.state.focused && <View
                    style={{height: 45 * k + 10.7 * k + (bot.address.suggestions.length ? 10.7 * k + bot.address.suggestions.length * 43.4 * k : 0)}}>
                    <ListView scrollEnabled={false} enableEmptySections={true} style={{paddingBottom: 10.7 * k}}
                              ...
                              renderRow={row => <TouchableOpacity key={row.id + 'vjew'}
                                  ...
                                  <View style={{
                                      ...
                                      <Text style={{
                                          ...
                                          fontFamily: 'Roboto-Regular',
                                          color: 'rgb(63,50,77)'
                                      }} numberOfLines={1}>{row.place_name}</Text>
                                      {/*<Text style={{width:75*k, paddingLeft:12*k}}>{row.distance}</Text>*/}
                                  </View></TouchableOpacity>}
                              renderSeparator={(s, r) => <View key={r + 'sep'}
                                                               style={{backgroundColor: 'rgba(255,255,255,0.9)'}}><Separator
                                  width={1}/></View>}
                              renderFooter={() => <View style={{paddingTop: 10.7 * k}}><TouchableOpacity
                                  onPress={() => alert('Follow me!')} style={{
                                  height: 45 * k,
                                  ...
                                  <Text style={{color: 'rgb(254,97,108)', fontFamily: 'Roboto-Bold',}}><Text>Follow
                                      me</Text> for 30 min</Text>
                              </TouchableOpacity>
            ...
            {this.props.onSave &&
            <Button buttonStyle={{position: 'absolute', bottom: 20 * k, left: 90 * k, right: 20 * k}}
                    onPress={() => this.props.onSave(bot.bot)}>Next</Button>}
        </View>
