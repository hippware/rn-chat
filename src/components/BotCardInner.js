          ...
          <View style={{flexDirection:'row'}}>
            <View style={{flex:1, height:75*k}}>
              ...
              <View style={{flexDirection:'row', paddingTop:10*k, alignItems:'center'}}>
                ...
                <View style={{paddingLeft:10*k}}><Image style={{width:14*k, height:17*k}} source={require('../../images/iconBotLocation2.png')}/></View>
                <Text style={{paddingLeft:10*k, paddingRight:10*k, fontSize:12,color:'rgb(155,155,155)',fontFamily:'Roboto-Regular'}}>{distance}</Text>
              </View>
            </View>
            <View style={{width:60*k}}>
              <View style={{flex:1}}/>
              <View style={{alignItems:'flex-end'}}>
                <Avatar size={30} profile={profile} source={profile.avatar && profile.avatar.source} tappable
                      title={profile.displayName} isDay={isDay} disableStatus borderWidth={0} />
              </View>
              <TouchableOpacity onPress={()=>statem.logged.profileDetailsContainer({parent:'_home', item: profile.user})}><Text style={{textAlign:'right', fontFamily:'Roboto-Regular', fontSize:13*k, color:'rgb(68,157,225)'}}>@{profile.handle}</Text></TouchableOpacity>
            </View>
          </View>
