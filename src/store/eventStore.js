...
import {action, autorun, toJS, runInAction} from 'mobx';
...
export class EventStore {
  ...
  @action
  processItem2 = async (item: Object, delay?: Object): Promise<boolean> => {
    const time = Utils.iso8601toDate(item.version).getTime();
    try {
      if (item.message && item.message.bot && item.message.bot.action === 'show') {
        const bot = botFactory.create({id: item.message.bot.id, server: item.message.bot.server, loaded: true});
        await botStore.load(bot); // bad bot should throw exception here, right?
        model.events.add(new EventBot(item.id, bot, time));
        console.log('& adding event bot');
        // model.events.add(new EventBot(item.id, item.message.bot.id, item.message.bot.server, time));
      } else if (item.message && item.message.event && item.message.event.retract) {
        log.log('& retract! ignoring', item.message.event.retract.id);
        // return false;
        ...
        let eventMessage;
        if (item.message.bot) {
          // NOTE: do we need need to async load this too?
          eventMessage = new EventBotShare(item.id, item.message.bot.id, item.message.bot.server, time, msg);
        } else {
          eventMessage = new EventMessage(item.id, msg.from, msg);
        }
  ...
  @action
  async accumulateItems(earliestId: string, current: number = 0, count: number = 3, pageSize: number = 5): Promise<void> {
    ...
    // console.log('& promise before', toJS(model.events.list));
    const processedArr = await Promise.all(data.items.map(i => this.processItem2(i)));
    // console.log('& promise processed', processedArr);
    current = processedArr.reduce((sum, value) => sum + value, current);
    // console.log('& new current', current);
    // console.log('& after promise', toJS(model.events.list));
    if (current < count) {
      console.warn('Homestream: some items arent being processed', model.events.list.length, count);
      await this.accumulateItems(earliestId, current, count, pageSize + 20);
    }
  }
  ...
  async request() {
    ...
    await Promise.all(data.items.map(i => this.processItem2(i)));
    const latest = data.items.find(i => i.version);
