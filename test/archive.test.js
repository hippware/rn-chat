...
describe('archive', () => {
  step('register/login user3', async () => {
  ...
  step('register/login user1', async () => {
  ...
  step('register/login user2', async () => {
  ...
  step('register/login user2 again', async () => {
    const data = testDataNew(9);
  ...
  step('delete user2', async () => {
    await profile.remove();
  });

  step('delete user3', async () => {
    const data = testDataNew(7);
    const {user, password, server} = await xmpp.register(data.resource, data.provider_data);
    const logged = await xmpp.connect(user, password, server);
    await profile.remove();
