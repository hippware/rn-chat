...
describe('BotStore', () => {
  ...
  it('update bot location', async done => {
    timestamp()
    try {
      await bot.update({public: true, location: {latitude: 1.3, longitude: 2.3}, title: 'Test bot!'})
      expect(bot.isNew).to.be.false
  ...
  it('create bot2', async done => {
    try {
      ...
      await bot2.update({public: true, location: {latitude: 1.2, longitude: 2.2}, title: 'Test bot2'})
