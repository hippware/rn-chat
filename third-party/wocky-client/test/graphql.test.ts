...
describe('GraphQL', () => {
  it('get user1 credential', async done => {
    try {
      timestamp()
  ...
  it('get user2 credentials', async done => {
    try {
      timestamp()
  ...
  it('login via graphql and set user location', async () => {
    timestamp()
    gql = new NextGraphQLTransport('next')
    await gql.login(user.password!, host)
