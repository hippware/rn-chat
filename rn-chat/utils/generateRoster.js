...
const uuid = '407ad12e-0e27-11e6-a26c-0ed7e4a33b15'; // steve
const sessionID = '$T$D/v9VGcsfckGxtOVH0jf4BFqZ7Bzu4XNXwnG2Xk8il8=';

describe('Generate avatars', () => {
  const avatars = getAvatars();
  for (let i = 0; i < avatars.length; i++) {
    step(`Create and login user${i}`, (done) => {
      const {data, ...avatar} = avatars[i];
      const test = {...testData(100 + i), ...avatar};
      verifyAction(
        actions.login(test),
        [
    ...
    step('subscribe me', (done) => {
      verifyAction(actions.subscribe(uuid), [{type: actions.SUBSCRIBE, user: uuid}], done);
    });
    step('send message to me', (done) => {
      const {data, ...avatar} = avatars[i];
      const msg = {body: `Hello from ${avatar.firstName} ${avatar.lastName}`, to: uuid};
      verifyAction(
        actions.sendMessage(msg),
        [
          {type: actions.MESSAGE, msg},
          {
