...
const profileStore = mobx_state_tree_1.types
    ...
    .named('Profile')
    .actions(self => {
    const registerProfile = (profile) => self.profiles.put(profile) && self.profiles.get(profile.id);
    const unregisterProfile = (user) => self.profiles.delete(user);
    return {
        registerProfile,
        unregisterProfile,
        createProfile: (id, data) => registerProfile(Object.assign({}, data, { id })),
        getProfile(id) {
        ...
        loadProfile: mobx_state_tree_1.flow(function* (user) {
            ...
            if (isOwn) {
                if (self.profile) {
                    ...
                    self.profile = model_1.OwnProfile.create(profile);
                }
                return self.profile;
            }
            else {
