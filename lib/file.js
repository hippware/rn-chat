...
exports.FileStore = mobx_state_tree_1.types
    .compose(register_1.default, mobx_state_tree_1.types.model('XmppFile', {
    files: mobx_state_tree_1.types.optional(mobx_state_tree_1.types.map(model_1.File), {})
}))
    .named('XmppFile')
    .actions(self => ({
    upload: mobx_state_tree_1.flow(function* ({ method, headers, url, file }) {
    ...
    return {
        createFile: (file) => {
            return self.files.get(file.id) || (self.files.put(file) && self.files.get(file.id));
        },
        ...
        requestUpload: mobx_state_tree_1.flow(function* ({ file, size, width, height, access }) {
