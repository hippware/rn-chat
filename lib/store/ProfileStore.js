...
const profileStore = mobx_state_tree_1.types
    ...
    return {
        loadProfile: mobx_state_tree_1.flow(function* (user) {
            ...
            if (isOwn) {
                self.profile = OwnProfile_1.OwnProfile.create(Object.assign({ id }, data, { status: 'available' }));
            }
            return res;
        })
    };
    ...
    return {
        _requestProfiles: mobx_state_tree_1.flow(function* (users) {
            ...
            return arr.map((user) => self.profiles.get(user.user, self._processMap(user)));
        }),
    ...
    let handler1 = null;
    return {
        afterCreate: () => (handler1 = mobx_1.autorun('ProfileStore', () => tslib_1.__awaiter(this, void 0, void 0, function* () {
