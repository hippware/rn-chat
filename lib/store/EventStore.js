...
function processHomestreamResponse(data, username, self) {
    ...
    return {
        list: items.map((rec) => processItem(rec, null, username, self)).filter((x) => x),
        bots,
...
function processItem(item, delay, username, self) {
    try {
        ...
        if (item.message) {
            ...
            if (bot && bot.action === 'show') {
                return self.create(EventBotCreate_1.EventBotCreate, { id, bot: bot.id, time, created: true });
            }
            if (bot && (bot.action === 'exit' || bot.action === 'enter')) {
                const userId = utils_1.default.getNodeJid(bot['user-jid']);
                return self.create(EventBotGeofence_1.EventBotGeofence, { id, bot: bot.id, time, profile: userId, isEnter: bot.action === 'enter' });
            }
            if (event && event.item && event.item.entry) {
                const { entry, author } = event.item;
                const eventId = event.node.split('/')[1];
                const post = self.create(BotPost_1.BotPost, { id: eventId + id, image: entry.image, content: entry.content, profile: utils_1.default.getNodeJid(author) });
                return self.create(EventBotPost_1.EventBotPost, { id, bot: eventId, time, post });
            }
            if (message['bot-description-changed'] && message['bot-description-changed'].bot) {
                ...
                return self.create(EventBotNote_1.EventBotNote, { id: item.id, bot: noteBot.id, time: utils_1.default.iso8601toDate(item.version).getTime(), note: noteBot.description });
            }
            if (event && event.retract) {
                return self.create(EventDelete_1.EventDelete, { id: event.retract.id, delete: true });
            }
            if (body || media || image || bot) {
                ...
                return bot ? self.create(EventBotShare_1.EventBotShare, { id, bot: bot.id, time, message: self.create(Message_1.Message, msg) }) : null;
            }
        }
        else {
...
exports.EventStore = mobx_state_tree_1.types
    ...
    _loadUpdates: mobx_state_tree_1.flow(function* () {
        ...
        bots.forEach((bot) => self.getBot(Object.assign({ id: bot.id }, self._processMap(bot))));
    ...
    return {
        afterCreate: () => {
            ...
            handler2 = mobx_1.autorun('EventStore notification', () => tslib_1.__awaiter(this, void 0, void 0, function* () {
                if (self.message && self.message.notification) {
                    try {
                        yield self._onNotification(self.message);
                    }
                    catch (e) {
