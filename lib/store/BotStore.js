...
exports.default = mobx_state_tree_1.types
    ...
    return {
        getBot: (_a) => {
            ...
            if (data && Object.keys(data).length) {
                self._registerReferences(Bot_1.Bot, data);
                bot.load(data);
            }
            return bot;
        },
    ...
    _loadOwnBots: mobx_state_tree_1.flow(function* (userId, lastId, max = 10) {
        ...
        for (const item of bots) {
            res.push(self.getBot(Object.assign({ id: item.id }, self._processMap(item))));
        }
    ...
    _loadBotPosts: mobx_state_tree_1.flow(function* (id, before) {
        ...
        return {
            ...
            list: res.map((x) => {
                ...
                const profile = self.profiles.get(utils_1.default.getNodeJid(x.author), {
                    handle: post.author_handle,
                    firstName: post.author_first_name,
                    lastName: post.author_last_name,
                    avatar: post.author_avatar
                });
                const result = self.create(BotPost_1.BotPost, {
                    id: post.id,
    ...
    loadBot: mobx_state_tree_1.flow(function* (id, server) {
        ...
        const data = yield self.sendIQ(iq);
        const res = self.getBot(Object.assign({ id: data.bot.id }, self._processMap(data.bot)));
        return res;
    }),
    ...
    _processGeoResult: (stanza) => {
        if (stanza.bot) {
            self.geoBots.put(self.getBot(Object.assign({ id: stanza.bot.id }, self._processMap(stanza.bot))));
        }
    }
}))
    .actions(self => {
