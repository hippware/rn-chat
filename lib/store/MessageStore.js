...
function processMessage(stanza, ownUserId) {
    ...
    console.log('PROCESS MESSAGE:', res, stanza);
    return res;
}
...
exports.default = mobx_state_tree_1.types
    ...
    onMessage: (msg) => {
        ...
        if (msg.body || msg.media || msg.image || msg.result) {
            try {
                const _a = processMessage(Object.assign({}, msg, { unread: true }), self.username), { chatId } = _a, message = tslib_1.__rest(_a, ["chatId"]);
                if (message.media) {
                ...
                self._addMessage(chatId, Message_1.Message.create(message));
    ...
    return {
        _sendMessage: (msg) => {
            ...
            self._addMessage(msg.to, mobx_state_tree_1.clone(msg));
        },
        ...
        loadChats: mobx_state_tree_1.flow(function* (max = 50) {
            ...
            items.forEach((item) => {
                ...
                if (from) {
                    const msg = Message_1.Message.create(processMessage(Object.assign({}, message, { to, from, time: utils_1.default.iso8601toDate(timestamp).getTime() }), self.username));
