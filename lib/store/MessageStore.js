    ...
    onMessage: (msg) => {
        ...
        if (msg.body || msg.media || msg.image || msg.result) {
            try {
                ...
                self._addMessage(chatId, self.create(Message_1.Message, message));
    ...
    return {
        ...
        loadChats: mobx_state_tree_1.flow(function* (max = 50) {
            ...
            items.forEach((item) => {
                ...
                if (from) {
                    const msg = self.create(Message_1.Message, processMessage(Object.assign({}, message, { to, from, time: utils_1.default.iso8601toDate(timestamp).getTime() }), self.username));
