...
exports.EventList = PaginableList_1.createPaginable(exports.EventEntity).actions(self => ({
    postProcessSnapshot: (snapshot) => {
        if (snapshot.result.length > 20) {
            const result = snapshot.result.slice(0, 20);
            const cursor = result[result.length - 1].id;
            return Object.assign({}, snapshot, { result, cursor });
        }
        return snapshot;
    }
}));
exports.Wocky = mobx_state_tree_1.types
    ...
    geofenceBots: mobx_state_tree_1.types.optional(Bot_1.BotPaginableList, {}),
    ...
    .actions(self => {
    const transport = mobx_state_tree_1.getEnv(self).transport;
    return {
    ...
    return {
        views: {
            ...
            get updatesToAdd() {
                return self.updates.filter((e) => {
                    return mobx_state_tree_1.getType(e).name !== EventDelete_1.EventDelete.name;
                });
            }
        },
        actions: {
    ...
    .views(self => ({
    get activeBots() {
        const arr = self.geofenceBots.list.filter((bot) => bot.visitorsSize).map((data, index) => ({ data, index }));
        return arr
            .sort((a, b) => {
            if (a.data.visitor && !b.data.visitor)
                return -1;
            if (!a.data.visitor && b.data.visitor)
                return 1;
    ...
    createProfile: (id, data = {}) => {
        return self.profiles.get(id, utils_1.processMap(data));
    },
    getBot: (_a) => {
    ...
    _loadOwnBots: mobx_state_tree_1.flow(function* (userId, lastId, max = 10) {
        ...
        return { list: list.map((bot) => self.getBot(bot)), count, cursor };
    }),
    _loadGeofenceBots: mobx_state_tree_1.flow(function* (lastId, max = 10) {
        yield utils_1.waitFor(() => self.connected);
    ...
    _loadBotSubscribers: mobx_state_tree_1.flow(function* (id, lastId, max = 10) {
        ...
        return { list: list.map((profile) => self.profiles.get(profile.id, profile)), count, cursor };
    }),
    ...
    _loadBotPosts: mobx_state_tree_1.flow(function* (id, before) {
        ...
        return { list: list.map((post) => self.create(BotPost_1.BotPost, post)), count, cursor };
    }),
    ...
    _shareBot: (id, server, recepients, message, action) => {
    ...
    _subscribeGeofenceBot: mobx_state_tree_1.flow(function* (id) {
        yield utils_1.waitFor(() => self.connected);
    ...
    _unsubscribeGeofenceBot: mobx_state_tree_1.flow(function* (id) {
        yield utils_1.waitFor(() => self.connected);
        return yield self.transport.subscribeBot(id, false);
    }),
    ...
    setLocation: mobx_state_tree_1.flow(function* (location) {
        return yield self.transport.setLocation(location);
    }),
    getLocationsVisited: (limit) => {
    ...
    _onBotVisitor: mobx_state_tree_1.flow(function* ({ bot, action, visitor }) {
        // console.log('ONBOTVISITOR', action, JSON.stringify(bot), visitor)
        const id = visitor.id;
        const botModel = self.bots.get(bot.id, bot);
    ...
    return {
        ...
        afterCreate: () => {
            ...
            mobx_1.reaction(() => self.transport.botVisitor, self._onBotVisitor);
        }
    };
});
